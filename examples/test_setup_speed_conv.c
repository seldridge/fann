...
int main()
{
	...
	fann_add_layer_type(layers, 0, FANN_INPUT_LAYER, num_input+i, 0, 0);
	fann_add_layer_type(layers, 1, FANN_CONVOLUTIONAL_LAYER, num_input+i, num_input+i, 0);
	fann_add_layer_type(layers, 2, FANN_MAXPOOLING_LAYER, 2+i, 2, 1);
	fann_add_layer_type(layers, 3, FANN_FCONNECTED_LAYER, num_output+i, 0, 0);

		 ann = fann_create_conv(num_layers, layers);
